...
DROP TABLE IF EXISTS sdp_categories;
DROP TABLE IF EXISTS categories;
DROP TABLE IF EXISTS trees;
DROP TABLE IF EXISTS alarm_distribution;
...
    run_id INTEGER,
    severity TEXT,
    alarm_type TEXT,  -- NEW: 'low_files', 'extra_category', 'missing_tree', etc.
    sdp_list TEXT,
    sdp_count INTEGER,
    tree_name TEXT,
    category_name TEXT,
    file_count INTEGER,  -- NEW: 0, 1, or more
...
CREATE TABLE alarm_distribution (
...
CREATE INDEX idx_runs_timestamp ON monitoring_runs(run_timestamp);
CREATE INDEX idx_alarms_run ON alarms(run_id);
CREATE INDEX idx_alarms_tree ON alarms(tree_name);
CREATE INDEX idx_alarms_type ON alarms(alarm_type);
CREATE INDEX idx_sdp_status_run ON sdp_status(run_id);
CREATE INDEX idx_trees_run ON trees(run_id);
CREATE INDEX idx_categories_run ON categories(run_id);
CREATE INDEX idx_categories_tree ON categories(tree_name);
CREATE INDEX idx_sdp_categories_run ON sdp_categories(run_id);
CREATE INDEX idx_sdp_categories_tree_cat ON sdp_categories(tree_name, category_name);
 
